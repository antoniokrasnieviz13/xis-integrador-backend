...
+    Aceita payload genérico e normaliza campos comuns.
+    """
+    db = _db_session()
+    try:
...
+        db.commit()
+        logger.info(
+            "Webhook recebido: order_id=%s external_code=%s itens=%s total=%.2f",
+            order.id,
+            external_code,
...
+    except Exception as e:
+        db.rollback()
+        logger.exception("Erro no webhook de pedidos: %s", e)
